...
export class UserController {
    ...
    refreshToken(@Req() request, @Res() res) {
        ...
        return this.userService.refreshToken({token, ipAddress})
    ...
    revokeToken(@Req() req) {
        ...
        return this.userService.revokeToken({token, ipAddress})
            .then(() => {
    ...
    async forgotPassword(@Req() req, @Body() body: ForgotPwdDto) {
        return  this.userService.forgotPassword(body, req.get('origin'))
            .then(() => {
                return {message: 'Инструкции по восстановлению пароля отправлены на почту.'}
            })
    }
    ...
    async validateResetToken(@Req() req) {
        return await this.userService.validateResetToken(req.body)
            .then(() => {
    ...
    async update(@Req() req, @Body() body: CrudUserDto) {
        ...
        let isAdminOrManager = ( req.user.roles.includes(Role.Manager) || req.user.roles.includes(Role.Admin) )
        let isManager = (req.user.roles.includes(Role.Manager))
        let changingAnotherUser = (req.params.id !== req.user.id)
        ...
        if (changingAnotherUser && isManager)
            body = _.pick(body, ['allowedBimCats'])
