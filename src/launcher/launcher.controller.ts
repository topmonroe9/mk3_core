...
export class LauncherController {
    ...
    async getById(@Param() param) {
        return this.launcherService.getById(param.id)
            .then( launchers =>{
                return launchers
            })
    }
    ...
    async downloadById(@Param() param, @Res() res) {
        return this.launcherService.downloadById(param.id)
            .then( file =>{
    ...
    async create(@Body() body) {
        return this.launcherService.create(body)
            .then( launcher => {
                return launcher
            })
    }
    ...
    async update(@Req() req) {
        return this.launcherService.update(req.params.id, req.body)
            .then( launcher => {
                return launcher
            })
    }
    ...
    async delete(@Req() req) {
        return this.launcherService.delete(req.params.id)
            .then( () => {
    ...
    private generateLink(fileName: string): string {
        return `public/launchers/${fileName}`
    }
}
